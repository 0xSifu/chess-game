...
export default class Scene extends Component {
    ...
    connectWallet = async (event) => {
        event.preventDefault(); // Prevent any default action that might open a new tab
        if (window.ethereum) {
            try {
                // Request account access
                const accounts = await window.ethereum.request({ method: 'eth_requestAccounts' });
                console.log('Connected account:', accounts[0]);
                this.setState({ wallet: accounts[0] }); // Update state with the connected wallet address

                // Optionally, you can fetch additional data here
                this.fetchAdditionalData(accounts[0]);
            } catch (error) {
    ...
    fetchAdditionalData = async (account) => {
        // Fetch any additional data you need using the connected account
        console.log(`Fetching additional data for account: ${account}`);
    }
...
/***********************************************************************************************/
        ...
        var mouseDownAction = function (event) {
            ...
            if( self.selectedPiece ) {
                if( self.props.mode === gameModes['P2P'] ) {
                    ...
                    self.selectedPiece.mesh.position.y = this.selectedPiece.currentY;
                    self.selectedPiece = null;
                }
            }
        };
    ...
    render() {
        const { wallet } = this.state; // Destructure wallet from state
        ...
        return (
          <div className="GameScene">
            <div style={{ margin: '20px' }}>
                    ...
                    >
                        {wallet ? wallet : "Connect Wallet"} {/* Display wallet address or "Connect Wallet" */}
                    </button>
                </div>
            ...
            <Claim 
                ...
                msg={`Congratulations, You won ${this.state.numConsecutiveWins} matches in a row. You earn ${this.state.bonusReward} LGG more!`} 
                onClickClaim={wallet ? this.onClickClaim : this.connectWallet} // Use connectWallet if wallet is not connected
                btnText={wallet ? wallet : "Connect Wallet"} // Show wallet address if connected
                disabled={!!wallet} // Disable the button if wallet is connected
            />
